...
class Reader(object):
    def __init__(self, fname):
        ...
        self.data_offset = self.meta.record_size * 2 // 8 * self.meta.node_count + 16
    ...
    def _read_search_tree_node_24(self, node_idx):
        ...
        return SearchTreeNode(
            self._idx_to_node(left_idx, divein_func),
            self._idx_to_node(right_idx, divein_func),
        )

    def _read_search_tree_node_28(self, node_idx):
        offset = node_idx * 7
        b1, b2, b3, b4, b5, b6, b7 = struct.unpack_from(">BBBBBBB", self.db, offset)
    ...
    def _unserialize(self, offset=None):
        ...
        if field_type == types.TYPE_POINTER:
            ...
            if ss_bits == 0:
                ...
                b1, b2, b3 = struct.unpack_from(">BBB", self.db, self.offset)
                self.offset += 3
                pointer = ((vvv_bits * 256 + b1) * 256 + b2) * 256 + b3 + 526336

            else:  # ss_bits == 3
                b1, b2, b3, b4 = struct.unpack_from(">BBBB", self.db, self.offset)
